...
describe("gallery: 対象idの体系(契約「対象idの体系」)", () => {
  it("対象idは一般建物の全6 role+中心建築+施設の全 7 kind", () => {
  ...
  it("未知の対象idは例外を投げる", () => {
    expect(() => buildGalleryWorld("building/unknown", "seed-x", {})).toThrow();
...
describe("gallery: 施設対象(facility/field・facility/pasture)", () => {
  ...
  it("field は畝(ground-patch)の列、pasture は単一パッチ+横木2本の柵", () => {
...
describe("gallery: 広場施設対象(facility/well・facility/stall)", () => {
  ...
  it("well: 広場 1 枚に井戸 1 基(採否強制)。footprint は広場 polygon 内", () => {
    ...
    // 造形(contracts/facilities.md「well(井戸)」節「構成」): 石環 fence×4 + 内面 + 柱2 + 轆轤 + 小屋根
...
describe("gallery: 風車・水車対象(facility/windmill・facility/watermill)", () => {
  ...
  it("windmill: 塔 + キャップ + ロータ(4 枚羽根・静的 phase)+ 扉・窓", () => {
    ...
    // ロータ直径 = 塔高の約 1.2 倍の誇張(contracts/facilities.md「windmill(風車)」節「ロータ」。±8% 揺らぎ込みの帯)
  ...
  it("watermill: 小屋 + 軸 + 水輪(下端が水面下)。footprint 頂点は陸上(水域例外)", () => {
...
describe("gallery: 桟橋対象(facility/pier)", () => {
  ...
  it("pier: 板 1 + 杭のみの構成。天端 0.35・先端が池の上に突き出す", () => {
...
describe("gallery: 中心建築対象(契約「中心建築対象の合成規約」)", () => {
  const CENTER_ID = "building/center";

  it("同一 seed + params で2回生成してハッシュ・内容が完全一致する", () => {
    const params: Partial<Params> = { monumentality: 70, arcana: 80 };
    const a = buildGalleryWorld(CENTER_ID, "everdusk-101", params);
    const b = buildGalleryWorld(CENTER_ID, "everdusk-101", params);
    expect(a.summary.hash).toBe(b.summary.hash);
    expect(hashWorldModel(a)).toBe(a.summary.hash);
    expect(JSON.stringify(a)).toBe(JSON.stringify(b));
  });

  it("対象1体が role center で生成され、centerPlan が合成規約どおり", () => {
    const model = buildGalleryWorld(CENTER_ID, "everdusk-101", {});
    expect(model.buildings.length).toBe(1);
    const building = model.buildings[0];
    expect(building).toBeDefined();
    if (!building) return;
    expect(building.role).toBe("center");
    expect(building.id).toBe("building/center");
    expect(building.parcelId).toBeNull();
    expect(building.parts.length).toBeGreaterThan(0);
    // centerPlan: 原点・軸平行正方形(一辺 = derived.centerFootprint)・
    // facing = −π/2
    expect(model.centerPlan.position).toEqual({ x: 0, z: 0 });
    expect(model.centerPlan.facing).toBe(-Math.PI / 2);
    const half = model.meta.derived.centerFootprint / 2;
    for (const v of model.centerPlan.footprint) {
      expect(Math.abs(Math.abs(v.x) - half)).toBeLessThan(1e-9);
      expect(Math.abs(Math.abs(v.z) - half)).toBeLessThan(1e-9);
    }
    // heightHint は段9 と同一式(centerHeight × 軸別係数)の正値
    expect(model.centerPlan.heightHint).toBeGreaterThan(0);
  });

  it("スカイライン契約: 仮想周辺最高点 9.0 × skylineRatio を満たす", () => {
    const model = buildGalleryWorld(CENTER_ID, "everdusk-101", {
      monumentality: 60,
    });
    const building = model.buildings[0];
    ...
    expect(top).toBeGreaterThanOrEqual(9 * model.meta.derived.skylineRatio);
  });

  it("水辺スライダー最大で waterside 軸が成立し得る(常設池 = 近接度 1)", () => {
    const model = buildGalleryWorld(CENTER_ID, "everdusk-101", {
      monumentality: 80,
      water: 100,
      prosperity: 0,
    ...
    expect(axes.waterside).toBeGreaterThan(axes.authority);
    expect(axes.waterside).toBeGreaterThan(axes.rustic);
  });

  it("既存ワールド生成(runPipeline)へ影響しない", async () => {
    const before = await runPipeline("everdusk-101", DEFAULT_PARAMS);
    buildGalleryWorld(CENTER_ID, "everdusk-101", {});
    buildGalleryWorld(CENTER_ID, "unrelated-seed", { monumentality: 90 });
    const after = await runPipeline("everdusk-101", DEFAULT_PARAMS);
    expect(after.summary.hash).toBe(before.summary.hash);
  });
});
