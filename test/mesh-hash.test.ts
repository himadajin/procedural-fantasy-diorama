...
// 位置ハッシュの返り値域の契約(0 ≤ h < 1。contracts/pipeline.md
// 「WorldModel 正規化ハッシュ」節)を機械検証する。符号付き 32bit の
// XOR 結果が負値になりうるため、最終演算後に `>>> 0` で符号なしへ
// 正規化してから除算する。負値が混じると明度ムラ `1 + AMP*(2h-1)` が
// ムラ振幅の帯域(±8%。art-direction 5.1/5.2節)を超えて暗くなる。
