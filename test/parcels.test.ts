...
describe("parcels: 区画グループ(groupId。契約「区画グループとクラスタパターン」)", () => {
  // residential は <slot>、farmland は farm<slot>(Phase D。
...
describe("parcels: 農地区画(Phase D。contracts/facilities.md「field(畑)・pasture(牧草地)」)", () => {
  ...
  it("Settlement=0 で全 seed に農地区画が 2 件以上、Settlement=50 でも合計 1 件以上生じる(D2a 差し戻しの完了条件)", () => {
    // 実測(D2a 差し戻し。6 seed): settle0 は 7〜18 件/seed、settle50 は
