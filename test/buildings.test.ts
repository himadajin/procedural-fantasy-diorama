...
describe("buildings: 骨格データの性質", () => {
  it("全採択 residential 区画がちょうど 1 棟に帰属する(単棟=1区画1棟、連棟=k区画1棟。spanParcelIds の全単射)、中心建築(role center)が末尾に 1 棟", () => {
    // 契約「連棟(rowhouse)の性質」による 1:1 の改訂: 建物は区画と
    // 単射ではなく、spanParcelIds による全単射(区画集合の分割)になる。
    // 契約「全単射の対象範囲」: この全単射は kind "residential" の区画のみを
    // 対象とする。kind "farmland" の区画は建物を持たない(下記の別テストで
    // 検証)。
    for (const seed of SEEDS) {
  ...
  it("farmland 区画は建物を 0 棟持つ(contracts/buildings.md「全単射の対象範囲」)", () => {
  ...
  /** footprint 重心での urbanity(市街度)。floors のサイズ勾配検証用 */
  ...
  it("サイズ勾配: 一般建物の 4 階は urbanity ≥ 0.75 の位置のみ許容される", () => {
  ...
  it("階数分布の再配分: settle100 の 2/3/4 階比率が目標帯に収まる", () => {
    // contracts/buildings.md「floors の改訂」節「階数分布の再配分」。
    // 代表 3 seed × Prosperity{0,50,100} 合算(同節記載の計測手法)。
    ...
    // 目標帯(contracts/buildings.md「floors の改訂」節「階数分布の再配分」)
...
describe("buildings: 同型連続の抑制(位置由来の決定論的変調)", () => {
  /**
  ...
  it("同一壁材が 3 連続する頻度が、変調なし(実測固定値)より有意に低い", () => {
    ...
    // (contracts/buildings.md「同型連続の抑制の設計原理」節「統計的実測」)。
    // 同一コードで序数変調(MATERIAL_PARITY_WAVE)を 0 に戻すと
    // windows=191 / run3plus=29(rate 0.152)になる。変調ありの実測は
    // 以下のとおり有意に下回る。
    let windows = 0;
    let run3plus = 0;
    for (const seed of SEEDS) {
...
describe("buildings: グループ抽選と雁行(contracts/buildings.md「区画グループとクラスタパターン」「雁行(stagger)の性質」)", () => {
  // 長いグループが多数出る条件(settle100 で最長 13)を複数 seed ×
  // Settlement/Prosperity で走査し、雁行(stagger)当選グループを
  // 十分な件数で観測する
...
describe("buildings: 中庭型(courtyard)(contracts/buildings.md「中庭型(courtyard)の性質」)", () => {
...
describe("buildings: 連棟(rowhouse)(contracts/buildings.md「連棟(rowhouse)の性質」)", () => {
    ...
    for (const seed of SEEDS) {
      for (const over of SWEEP) {
        ...
        for (const b of rowhouses(model)) {
          ...
          // 裏庭 shed の wall/gable(params.backyard=1)は
...
describe("buildings: 裏庭(backyard)(contracts/buildings.md「裏庭の性質」)", () => {
  ...
  it("密集地(settle100)でも裏庭が一定割合成立する(契約「単棟・雁行の帯確保」の奥行き絞り再生成)", () => {
    // 帯確保の奥行き絞り再生成なしでは settle100 の裏庭発生率は 0.5%
    // (帯 2.5 が浅い区画でほぼ成立しない)。絞り再生成後の実測は settle100 の
