...
| タスク | 状態 | 備考 |
|---|---|---|
| N0 規範追加と計画書 | 完了 | 0015e29 |
| N1 docs 受け皿整備と文書追随 | 完了 | 54fc619(差し戻しなし) |
| N2 src/ 歴史語彙の全面除去 | 完了 | b97d4fa(37 ファイル。差し戻しなし) |
| N3 test/ 歴史語彙の除去と改名 | 完了 | 2d66638(27 ファイル。チェンジログ 659 行削除) |
| N4 施設 door/window 描画修正 | 完了 | 38790f9(回帰テスト 3 件追加) |
| N5 テストヘルパー集約 | 完了 | 4e5c17c(20 ファイル移行、−602 行) |
| N6 UI 文言・半開シート追随 | 完了 | 960efdd + レビュー修正 028f74a |
| 検収(受け入れ基準 1〜5) | 完了 | 下記「検収記録」 |
| N7 決定性バグ修正(分岐反転) | 完了 | 7877ba9。develop 初回 CI(x86_64)失敗を受けて追加。arm64・Docker x86_64 両方で SNAPSHOT 完全一致を確認済み |

### 検収記録(2026-07-15)

- 基準 1(歴史語彙 grep): src/ + test/ で 0 件。
- 基準 2(SNAPSHOT 不変): test/hash.test.ts の 8 エントリ(seed・params・
  ハッシュ値)を develop(3206c38)と機械照合し完全一致。
- 基準 3(4 コマンド): npm test 32 ファイル / 440 件全緑(smoke 1 件削除・
  mesh-facilities 3 件追加で 438 → 440)、typecheck・lint・build 全緑。
- 基準 4(console warn 0): dev サーバ実機でデフォルト seed 生成・ギャラリー
  風車/水車(URL 直リンクと Generate 再生成)を console.warn フック計測で
  warn 0 件を確認。風車の扉の描画を目視確認。水車の開口は同一関数
  (openingPart)を通るためコード照合で等価性を確認。
- 基準 5(参照整合): コード中の全「文書.md「節名」」引用(約 40 種)を
  実文書との機械照合で全一致(不一致 4 件を検収時に発見し fa34b4a で修正)。
- レビュー時のオーケストレーター直接修正(workflow.md の数行規定内): (1)
  N6 の半開シートで .pfd-gallery-actions を行ごと隠すとギャラリーの
  Generate まで消え art-direction 9.4 に反したため、非表示対象を
  Reroll(.pfd-ghost-btn)のみへ修正(028f74a)。(2) 引用文言の厳密化
  (fa34b4a。上記基準 5)。
- モバイル半開シート(375×812)でタブ + seed + Randomize + Generate のみの
  表示になることを実機確認。
