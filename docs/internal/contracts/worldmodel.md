...
PHASE 4b 以降の部品型(window / door / beam / chimney / deck / stair /
dormer / spire / crystal など)は、この2経路のどちらかへ型ハンドラの
...
| `wood` | `#5d4a38` | 木部(杭。4b で梁・窓枠・扉・欄干) |
| `thatch` | `#97855a` | 茅屋根 |
| `shingle` | `#7a6a52` | 木羽屋根 |
| `tile` | `#a0563c` | 瓦屋根 |
| `slate` | `#5c6b7a` | スレート屋根 |
...
`Building.materials` の決め方(段12 の commit 13 分):

- `wall` ∈ { `rough-wood`, `plaster`, `stone`, `ashlar` }。
  `derived.wallTier`(0〜3)+役割補正(hall +0.6 / bridgehead +0.2 /
  waterside −0.2 / warehouse −0.5 / outskirt −0.7 / house 0)+
  seed 揺らぎ(±0.6)を丸めて添字にする
- `roof` ∈ { `thatch`, `shingle`, `tile`, `slate` }。`derived.roofTier` から
  wall と同じ式(役割補正・揺らぎも同係数)
- `trim` ∈ { `wood`, `stone` }。壁が石造系(`stone` / `ashlar`)なら
  `stone`、それ以外は `wood`
- 壁材階層・屋根パレットの本格移行(Prosperity 3 水準の見た目検証と
  開口・木組みとの連動)は PHASE 4b の担当。本 commit は 5.1節の
  基準色の選定までを行う
...
建物部品の性質(段12「建物」の commit 13 分が保証し、メッシュビルダー・
テストが前提としてよい):
...
- 屋根の勾配は `roof.pitch`(50〜58度)を params.pitch にそのまま持ち、
  ...
  スパン側 0.65・妻側 0.4(深い軒。art-direction 6節)で、部品が
  footprint から 0.75 を超えてはみ出さない
