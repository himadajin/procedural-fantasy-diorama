...
Phase E の追加語彙候補(中心建築の建築群化。`../plans/2026-07-20-center-complex.md`
3.6節。**まず既存型の合成で作り、合成で成立しない場合のみ採用する**。
採否はタスク E3 で確定し、不採用の行は本表から削除する):

| type | 経路(候補) | 形状 | scale の意味 | params |
|---|---|---|---|---|
| `battlement` | インスタンス | 胸壁(壁・塔の天端の凸列。城郭型の囲い・隅塔用)。採用時は 1 凸 = 1 ピースのピース分解(「インスタンス経路のピース分解」の流儀)を基本とし、凸ピッチを実寸固定に保つ | 長さ×凸高×厚み | なし |
| `lean-to` | マージ(屋根束) | 片流れ屋根(回廊・下屋用の傾斜面+軒裏)。Phase C 裏庭の構成概念「shed(小屋)」との用語衝突を避けるため lean-to と命名 | 長さ×高さ×スパン。ローカル −z 側が高い | `pitch`: 勾配(度) |
| `cone` | マージ(屋根束) | 円錐屋根(隅塔用の多角錐+軒裏) | 底径×高さ×底径 | なし |
...
> **Phase E 移行中の註**: 本節は Phase E
> (`../plans/2026-07-20-center-complex.md`)の改訂後契約である。
> 実装はタスク E3〜E4 で追随する(それまでコードは旧契約の挙動)。
> 実装完了時に本註を削除する。
