...
> Phase B の決定であり、タスク B3(street ブースト)・B5(zoning)で
> 実装済み(2026-07-12)。
...
- 消費者: Phase D(施設。畑・牧草地の配置は農村ゾーンに置く)。Phase B
  ...
  vegetation(段15。Phase D の段14「施設」挿入で繰り下げ)の農村ゾーンでの内地散布量調整は B6 の裁量に含めてよい
  (任意)
...
> street 近接ブースト・zoning との対応は Phase B の決定であり、
> タスク B2(進入点数)・B3(street 本体)・B5(zoning 一本化)で
> 実装済み(2026-07-12)。
...
- **成長**: 決定論的な優先度キュー(優先度 = 成長優先度場の値、同値は
  ...
  値に比例する
- **発芽・分岐のキック角(70°〜110°。提案値)**: 発芽初期方向・分岐方向は
  親道路(幹線 edge の接線、または分岐元 street の直前セグメント方向)に
  対しほぼ直角のキックを与える。親と同じ方向(0°付近)へ発芽・分岐すると
  親道路と平行に重複するだけで新しい街区を区切れないため、ほぼ直角に
  振ることで発芽・分岐のたびに既存道路から仕切られた新しい街区が生まれ、
  進入点→中心のスポークだけでは埋まらない市街内部を街区状に充填できる
  (契約に数値の定めがなかったための実装判断)
