...
- ハッシュの一致は**同一 JS エンジン内でのみ保証**する。超越関数
  (`Math.sin` 等)の実装は engine ごとに末尾ビットが異なりうるため、
  異なる engine(例: 異なるブラウザ)間でのハッシュ一致は原理的に
  保証しない。数値を小数6桁へ丸めてから文字列化する手順(上記2.)が
  この差を実用上ほぼ吸収するが、無条件の保証ではない。
