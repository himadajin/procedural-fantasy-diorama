...
- 中心建築 `building/center`(role 体系に合わせる。Phase E タスク E1 で
  実装 2026-07-20。合成規約は下記「中心建築対象の合成規約」)。
- 将来の拡張(本計画のG0では実装しない。対象対応表に行を足すのみで
  成立させる): 施設 `facility/<kind>`(Phase D。
...
**中心建築対象の合成規約(Phase E タスク E1。2026-07-20)**:
`building/center` の極小ワールドは `centerPlan` を段4(立地)・段9(広場)
の確定値に相当する最小限で合成し、本番の `expandCenterBuilding`
(`pipeline/center`)のみを呼んで対象を 1 棟生成する。

- `centerPlan.position` = 原点、`footprint` = `derived.centerFootprint` を
  一辺とする軸平行正方形、`facing` = −π/2(他対象と同じ「正面が順光」の
  向き)
- `axes` は段4 と同一式・同一ストリーム(`"siting/axes"`。
  implementation-spec 1.7節)で合成する。水辺近接度は 1 とする(下記の
  常設池があるため)
- `heightHint` は段9 と同一式(`derived.centerHeight ×
  AXIS_HEIGHT_FACTOR[支配軸]`)で合成する
- 背面(+z)に池を常設する(waterside 軸の成立と水際造形 — 水面
  プローブ・接地契約 — のため。`centerPlan.footprint` と重ならない距離に
  置く)
- スカイライン契約の入力には、**仮想の周辺最高点 9.0**(一般建物の
  代表値 = 2 階+屋根相当)を表す合成 Building 1 棟を
  `expandCenterBuilding` の引数として与える(`model.buildings` には
  含めない)。周辺一般建物の無い極小ワールドでも、塔と群の比率が本番に
  近づくようにするための合成入力である
- 中庭 Plaza は `expandCenterBuilding` が本番どおり `model.plazas` へ
  追記する
- **関連パラメータ**: 中心建築対象は Prosperity / Settlement / Water /
  Arcana / Monumentality の 5 キー(4軸スコアと派生値に効くもの。
  他対象は Prosperity / Settlement の 2 キーのまま)。URL・UI スライダーの
  表示は対象idごとにこのキー集合を共用する(`src/ui/gallery-url.ts` の
  `galleryUrlParamKeys`)
